...
#let question_block(number: int, body: str) = {
  return block(
    stroke: 1pt, inset: 8pt, fill: rgb(245, 245, 245), radius: 4pt,
  )[#strong("Question " + str(number)): #text(body, font: "IBM Plex Serif", size: 9pt)]
}
#align(center)[
...
#question_block(
  number: 1, body: "Is your browser running HTTP version 1.0 or 1.1? What version of HTTP is the server running?",
)

*Answer:* We can observe based on the image and the response as well as request,
...
#question_block(
  number: 2, body: "What is the IP address of your computer? Of the www.cas.mcmaster.ca server?",
)
...
#question_block(
  number: 3, body: "What is the status code returned from the server to your browser?",
)

*Answer*: The status code was a 200 OK message.

#figure(
  image("assets/status_code_200_ok.png", width: 80%), caption: [Status code of GET Response],
) <fig-status_code_200_ok>
#question_block(
  number: 4, body: "When was the HTML file that you are retrieving last modified at the server?",
)
...
#question_block(
  number: 5, body: "How many bytes of content are being returned to your browser?",
)
...
#question_block(number: 6, body: "How many HTTP GET requests were sent?")
...
#question_block(number: 7, body: "How many HTTP response messages were received?")
...
#question_block(
  number: 8, body: "If multiple response messages were received, what are the difference between them?",
)
...
#question_block(
  number: 9, body: "If instead of clicking the \"refresh\" button, you re-enter the URL in the browser or simply enter return when the URL is highlighted, answer Q6 – Q8.",
)
...
#question_block(
  number: 10, body: "How many HTTP GET request messages did your browser send? Which packet number in the trace contains the GET message for the Bill or Rights?",
)
...
#question_block(
  number: 11, body: "Which packet number in the trace contains the status code and phrase associated with the response to the HTTP GET request?",
)
...
#question_block(
  number: 12, body: "What is the status code and phrase in the response?",
)

The status code and phrase in the response is a 200 OK

#question_block(
  number: 13, body: "How many data-containing TCP segments were needed to carry the single HTTP response and the text of the Bill of Rights?",
)
...
#question_block(
  number: 14, body: "What is the server's response (status code and phrase) in response to the initial HTTP GET message from your browser?",
)
...
#question_block(
  number: 15, body: "When your browser's sends the HTTP GET message for the second time, what new field is included in the HTTP GET message?",
)
